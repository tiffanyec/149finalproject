		...
		for state in dfa.states:
			if i == n:
				...
				if isAdversarial(state, i):
					try:
						count = min((lc[transitions[symbol]] if symbol in transitions else 0 for symbol in alphabet))
					except:
						count = 0
				else:
	...
	def improviser(si=i.initialState, sa=a.initialState, k=0):
		...
		while si != None and sa != None and k < n:
			...
			if adversarialI(si, k):
				...
				probWeights = [w/sum(weights) for w in weights]
				idx = random.choice(range(len(alphabet)), p=probWeights)
				symbol, mi, ma = tuple(zip(alphabet, iweights, aweights))[idx]
